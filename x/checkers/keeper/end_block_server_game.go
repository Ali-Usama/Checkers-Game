...
func (k Keeper) ForfeitExpiredGame(gotCtx context.Context) {
	...
	for {
		...
		if deadline.Before(ctx.BlockTime()) {
			...
			if storedGame.MoveCount <= 1 {
				// No point in keeping a game that wasn't played
			...
			// Move along FIFO for next iteration
			gameIndex = systemInfo.FifoHeadIndex
		} else {
			// all other games after are active
			break
		}
	}
